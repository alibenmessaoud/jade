...
public class ThanksAgent extends Agent {
  ...
  */
		...
		// create a new agent
		 Agent t1 = ( Agent) ac.createNewAgent(t1AgentName,getClass().getName(),new Object[0]);
		// fire-up the agent
		((PlatformController) t1).start();
		System.out.println(getLocalName()+" CREATED AND STARTED NEW THANKSAGENT:"+t1AgentName);
		// create a new agent
		 Agent t2 = ( Agent) ac.createNewAgent(t2AgentName,getClass().getName(),new Object[0]);
		// fire-up the agent
		((PlatformController) t2).start();
		System.out.println(getLocalName()+" CREATED AND STARTED NEW THANKSAGENT:"+t2AgentName);
	    } catch (Exception e2) {
		e2.printStackTrace();
	    }
